...
export function buildSystemPrompt(input: {
  ...
  const lines: string[] = [
    ...
    "Не выдумывай вещи, которых нет в гардеробе. Если гардероб пуст — попроси добавить фото. Если погоды нет — попроси геолокацию или город в профиле.",
    "",
  ...
  if (weather) {
    ...
    lines.push(
      profile.city?.trim()
        ? "Не удалось получить погоду. Спроси уточнить город или ориентируйся на описание мамы."
        : "Погода ещё не определилась (нет города и геолокации). Попроси разрешить местоположение в браузере или указать город в профиле — не выдумывай °C.",
    );
  }
