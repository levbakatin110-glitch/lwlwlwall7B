...
export function buildSystemPrompt(input: {
  ...
  const lines: string[] = [
    ...
    "Не выдумывай вещи, которых нет в гардеробе. Если гардероб пуст — попроси добавить фото. Если погоды нет — попроси нажать «Определить по GPS» над чатом. НИКОГДА не проси ввести город вручную.",
    "",
  ...
  if (weather) {
    ...
    lines.push(
      "Погода ещё подтягивается по местоположению. НЕ проси город текстом. Скажи коротко: «нажмите «Определить по GPS» над чатом» или подождите секунду и спросите снова. Не выдумывай °C.",
    );
  }
