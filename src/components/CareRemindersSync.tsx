...
export function collectScheduledPushes(now = Date.now()): ScheduledPushItem[] {
  ...
  for (const [childId, space] of Object.entries(s.childSpaces ?? {})) {
    Object.assign(journals, space.journals ?? {});
    const child = s.children.find((c) => c.id === childId);
    const name = childDisplayName(child);
    const spaceReminders = (space.careReminders ?? [])
      .map(sanitizeReminder)
      .filter((row): row is CareReminder => Boolean(row));
    reminders.push(...spaceReminders);
    for (const r of spaceReminders) {
      if (r.kind !== "custom" || !r.enabled) continue;
      ...
      items.push({
        ...
        url: r.href || "/reminders",
        tag: `${childId}:${r.id}`,
        nextAt: computeNextAt(r, now, tz, last),
        mode: r.mode,
  ...
  const name = childDisplayName(active ?? s.children[0]);
  ...
  for (const [i, slot] of planned.entries()) {
    const stub: CareReminder = {
      id: `daily-${i}`,
      kind: "custom",
      enabled: true,
      mode: "times",
      times: [slot.at],
      title: slot.cta.title,
      body: slot.cta.body,
      href: slot.cta.href,
      quietFrom: "21:00",
      quietTo: "08:00",
    ...
    items.push({
      ...
      url: slot.cta.href,
      tag: `daily:${i}`,
      nextAt: computeNextAt(stub, now, tz, null),
      mode: "times",
      times: [slot.at],
      quietFrom: "21:00",
      quietTo: "08:00",
